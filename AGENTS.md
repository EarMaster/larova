...
| `CHANGELOG.md` | [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) history. **Update `## [Unreleased]` in the same commit as any user-facing change.** `release.yml` extracts GitHub Release notes from the `## [x.y.z]` heading matching `versionName`, so headings must stay exact. **No column limit and no hard-wrapped bullets** — one bullet is one line, however long: GitHub renders a release body with hard line breaks, so a hand-wrapped bullet arrives on a phone as a paragraph broken mid-sentence |
