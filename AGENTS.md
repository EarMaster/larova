...
| `core/ui/icons/` | The sixty-eight tile symbols as SVG — `lucide/` vendored under ISC, `larova/` drawn here. Turned into Compose `ImageVector`s by the `generateTileSymbols` task; the file name is the frozen symbol key. See that folder's `README.md` |
