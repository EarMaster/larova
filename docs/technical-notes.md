...
Which text a caregiver is shown is `resolveCardText` in `:core:domain` — exact tag, then primary subtag with a tie-break that depends on neither row order nor timestamps, then the tile's own text. **Nothing filters.** A tile with no translation for the language somebody asked for shows what the parent wrote; hiding it would be indistinguishable from the tile never having existed, and the tile that vanished would be as likely to be the one about choking as any other. A variant written before the tile was last edited is shown too, and **not** marked: the flag is computed and tested, and nothing displays it — see `docs/localization.md` §1. Step 1 of that resolution — the language the original was written in — was unreachable until the editor learned to ask for `Card.locale`, and is now what lets somebody choose the original by name rather than only by turning the preference off.
