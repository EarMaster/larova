...
| In-app product `app.larova.unlock` created in Console → Monetise → Products | **yours** — a one-time product, non-consumable. The internal-track release already satisfies Play's "app must be on a track" prerequisite, so this can be done now |
