...
| In-app product `app.larova.unlock` created in Console → Monetise with Play → Products → One-time products | **yours, and it cannot be done before 0.4.0 is on a track.** Play only opens the products section once a build *containing the Play Billing Library* has been uploaded, and the AAB currently on internal (versionCode 6) predates it. So the order is: merge, let `release.yml` put versionCode 7 on internal, then create the product. Non-consumable, and the id must match `UNLOCK_PRODUCT_ID` exactly |
| **Family Library** enabled on that product | **yours** — one purchase then covers a household. Without it, the honest answer to "I have three children with three phones" is "buy it three times", which costs more goodwill than any shared backup file |
