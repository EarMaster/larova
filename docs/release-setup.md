...
| Play Console account, app created as `app.larova` | done — implied by the internal-track upload below |
...
| Build on the internal track | done — `0.4.0` (versionCode 7) was published by `release.yml` on 2026-09-01 and is the first build carrying the Play Billing Library. The first upload before it was the manual `0.3.2` that the listing edit attaches to |
...
| Content rating questionnaire retaken | done, 2026-09-03 — the IARC questionnaire asks about in-app digital purchases, and Play requires a fresh submission whenever a change would alter an answer |
| Right of withdrawal declared on the product | done, 2026-09-03 — `withdrawalRightType` = `WITHDRAWAL_RIGHT_DIGITAL_CONTENT` on the `unlock` purchase option, set through the API. It governs what Play tells an EU buyer about the fourteen-day cancellation right, and it was left unspecified when the product was first created. `productTaxCategoryCode` stays unset on purpose: unset means standard digital goods, which is the right category for an app unlock |
| App access instructions | **no credentials exist to give.** There is no account, no sign-in and no server, so the declaration is a short explanation rather than a login. The text below is what goes in the field |
| Data safety form reviewed for the purchase | **yours** — Google processes the payment and the app sends nothing, but the declaration is worth re-reading rather than left as it was |

## 6. App access, as declared to Play
